defmodule Traceroute.Ping do
  ...
  defp do_send(:icmp, ip, opts) do
    ...
    {:ok, pid_1} = Traceroute.Icmp.start_link([])
    {:ok, pid_2} = Traceroute.Icmp.start_link([])
    response_1 = Traceroute.Icmp.send(pid_1, packet, ip, opts.ttl, opts.timeout)
    # _response_2 = Traceroute.Icmp.send(pid_2, packet, ip, opts.ttl, opts.timeout)
    :ok = Traceroute.Icmp.stop(pid_1)
    :ok = Traceroute.Icmp.stop(pid_2)

    with {:ok, time, reply_packet} <- response_1 do
      {:ok, decode(reply_packet, time)}
    end
  end

  defp do_send(:udp, ip, opts) do
    {:ok, pid} = Traceroute.Udp.start_link([])
    response = Traceroute.Udp.send(pid, "probe", ip, opts.ttl, opts.timeout)
    :ok = Traceroute.Udp.stop(pid)
  ...
  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  #
  def encode(type, code, id, sequence, payload) do
